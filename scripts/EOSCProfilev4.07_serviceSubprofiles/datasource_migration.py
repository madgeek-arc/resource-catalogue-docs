    ...
    for file in os.listdir(directory + datasourceFolder):
        if file.endswith('.json'):
            ...
            with open(directory + datasourceFolder + file, 'r') as json_file:
                json_data = migrate_to_datasource(json_file, isVersion)
                # write to file
...
def migrate_to_datasource(json_file, isVersion):
    ...
    datasourceStatus = ET.Element("tns:datasourceStatus")
    if status is not None:
        match status:
            case "approved resource":
                datasourceStatus.text = "approved datasource"
            case "rejected resource":
                datasourceStatus.text = "rejected datasource"
            case "pending resource":
                datasourceStatus.text = "pending datasource"
        tree.append(datasourceStatus)
        tree.remove(status)

    # remove resourceExtras field
    resourceExtras = ET.Element("tns:resourceExtras")
    ...
    # change core ID
    coreId = create_new_id()
    json_data['id'] = coreId
    if isVersion:
        json_data["resource"]["id"] = coreId
        print("After the execution .json file must change from", json_data['id'], "to", coreId)
    else:
        print("After the execution .json file and -version folder names must change from", json_data['id'], "to", coreId)
