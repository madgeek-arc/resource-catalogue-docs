...
def folder_selection(directory):
    migrationFolders = ['/provider/']  # TODO: check Catalogue folder too
...
def migrate(json_file, isVersion):
    ...
    # delete legalStatus if it is set to 'Other'
    legalStatus = provider.find('{http://einfracentral.eu}legalStatus')
    if legalStatus is not None:
        if legalStatus.text == 'provider_legal_status-other':
            provider.remove(legalStatus)
