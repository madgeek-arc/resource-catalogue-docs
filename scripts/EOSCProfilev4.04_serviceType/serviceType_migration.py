...
def folder_selection(directory):
    migrationFolders = ['/service/', '/datasource/', '/pending_service/', '/pending_datasource/']
...
def migrate(json_file, isVersion):
    ...
    newServiceType = ET.Element("tns:serviceType")
    newServiceType.text = serviceType

    # add serviceType in resourceExtras
    resourceExtras = root.find('{http://einfracentral.eu}resourceExtras')
    if resourceExtras is None:
        newResourceExtras = ET.Element("tns:resourceExtras")
        newResourceExtras.append(newServiceType)
        tree.append(newResourceExtras)
    else:
        resourceExtras.append(newServiceType)
